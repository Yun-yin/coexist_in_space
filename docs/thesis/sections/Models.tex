...
\subsection{Generalized Lotka--Volterra Model}
    ...
    \end{equation}
    ensures the existence of a fixed point at which all species have positive abundances. Second, the local stability condition
    \begin{equation}
        \max_i \operatorname{Re} \left[ \lambda_i(\boldsymbol{M}) \right] < 0,
    \end{equation}
    where $\lambda_i(\boldsymbol{M})$ are the eigenvalues of the Jacobian matrix $\boldsymbol{M}$ evaluated at $\boldsymbol{N}^*$, guarantees that the equilibrium is locally asymptotically stable. Only parameter combinations that satisfy both conditions are considered to correspond to stable ecological communities.

\subsection{Diffusion-Reaction Model}

    In addition to species interactions, spatial community dynamics must account for the dispersal of individuals. Dispersal is commonly modeled using spatial derivatives of population density, which leads naturally to partial differential equation (PDE) formulations. A widely used approach is the diffusion model, in which dispersal is represented by a second-order spatial derivative. This formulation assumes that individuals move randomly and symmetrically in space. While such an assumption may allow for unrealistically rapid spread in some cases, it provides a tractable baseline description.
    ...
    In this study, the generalized Lotka--Volterra model is adopted as the reaction term. The spatial domain is defined as $[-1,1]$. We analyze in detail the cases of one- and two-species systems, and further consider a representative multi-species scenario to illustrate more complex dynamical behaviors.

    \subsubsection{One Species Invading Process}

        For a single species, the reaction--diffusion system reduces to a single partial differential equation,
        \begin{equation}\label{eqn:single_species}
            \frac{\partial u}{\partial t} = D \frac{\partial^2 u}{\partial x^2} + f(u),
        \end{equation}
        where $u(x,t)$ denotes the population density. We impose a step-like initial condition,
        \begin{equation*}
            u(x,0) =
            \begin{cases}
                1, & x < 0, \\
        ...
        When $f(u) = u(1-u)$, the model corresponds to the Fisher--KPP equation. The reaction term has two equilibria, $u=0$ and $u=1$. Linear stability analysis shows that $u=0$ is an unstable steady state, whereas $u=1$ is stable. Consequently, the interface (or invasion front) connecting the populated state ($u=1$) and the empty state ($u=0$) propagates toward the unstable state.

        Assuming a traveling wave solution of the form $u(x,t) = u(z)$ with $z = x - ct$, the governing equation reduces to the second-order ordinary differential equation
        \begin{equation*}
            \frac{\mathrm{d}^2 u}{\mathrm{d} z^2} + c \frac{\mathrm{d} u}{\mathrm{d} z} + u(1-u) = 0.
        \end{equation*}
        The existence of traveling wave solutions requires that the wave speed satisfies $c \geq 2$. Although closed-form solutions are generally not available, the leading-order approximation can be expressed as a logistic function,
        \begin{equation}
        ...
        To obtain a stationary front, the system must admit two stable equilibria. A representative example is the Allen--Cahn model with reaction term $f(u) = u(1-u)(u-a)$. The additional $(u-a)$ factor can be interpreted as an Allee effect, reflecting the dependence of growth on population density. This system has two stable steady states, $u=0$ and $u=1$, and one unstable state $u=a$.

        The direction and speed of front propagation are determined by the associated potential function $W(u)$, defined via $\frac{\mathrm{d} W}{\mathrm{d} u} = f(u)$. Evaluating the potential yields $W(0)=0$ and $W(1)=\frac{1}{12} - \frac{a}{6}$, from which the wave speed can be expressed as $c = \sqrt{2}\left(a - \frac{1}{2}\right)$. In this formulation, the propagation dynamics are fully determined by the parameter $a$, and the front becomes stationary when $a = \frac{1}{2}$.

        In this case, the traveling wave solution admits an explicit form,
        \begin{equation}
            u(z) = \frac{1}{1 + e^{z/c}},
        \end{equation}
        which corresponds exactly to a logistic profile.

    \subsubsection{Two Species Interaction--Dispersal Community}

        When interspecific interactions are considered, a two-species system provides the simplest framework to capture their effects. A reaction--diffusion model with a generalized Lotka--Volterra (GLV) reaction term can be written as
        \begin{equation}\label{eqn:two_species}
            ...
            \end{aligned}
        \end{equation}
        To illustrate competitive exclusion and spatial invasion dynamics, we focus on a parameter regime in which the system is locally bistable. Specifically, the intrinsic growth rates are taken to be positive, and interspecific competition is sufficiently strong to prevent local coexistence.
        ...
        The nondimensional system is characterized by four key parameters: $D$, $r$, $a_{1,2}$, and $a_{2,1}$. We first consider a symmetric case with $D = 1$, $r = 1$, and $a_{1,2} = a_{2,1}$, for which approximate analytical insights can be obtained under suitable assumptions on total population density. For the general case, numerical simulations are performed across a range of parameter combinations.

        Specifically, we explore the effects of dispersal and growth asymmetry by varying the diffusion ratio $D \in (0,2)$ and the relative growth rate $r \in (0,2)$, using the model
        \begin{equation*}\label{eqn:balance_D-r}
            \begin{aligned}
                \partial_t N_1 &= D \, \nabla^2 N_1 + N_1(1 - N_1 - 1.5 N_2),\\
                \partial_t N_2 &= \nabla^2 N_2 + N_2(1 - N_2 - 1.5 N_1).
            \end{aligned}
        \end{equation*}
        In addition, we investigate the influence of interspecific competition by varying $D \in (0,2)$ and $a_{1,2} \in (0,2)$, with the system
        ...
        The effects of intrinsic growth rates and competition strength are analyzed separately, as these parameters primarily govern local dynamics and stability properties of the GLV system. To quantify invasion outcomes, we measure the interface position $x^*$ defined by $u(x^*) = v(x^*)$ at the final simulation time, which serves as an indicator of propagation speed and coexistence behavior under different parameter regimes.
    ...
    \subsubsection{Competing Multispecies Community}
        ...
        We focus on bistable community configurations, in which the full system does not admit a stable coexistence of all species, but instead supports two alternative stable community states. These states correspond to two subsets of species, denoted as community $1$ and community $2$, each of which is internally stable. The dynamics can be reformulated by partitioning species into these two groups:
        \begin{equation*}
            \begin{aligned}
    ...
    \subsubsection{IMEX Semi-Implicit Method}
        ...
        \end{equation*}
        Substituting this into the IMEX formulation yields a linear system for $\mathbf{u}_{\ell+1}$ at each time step. The resulting system can be written in matrix form and solved efficiently using sparse linear algebra techniques.

        Compared to the explicit scheme, the IMEX method requires solving a linear system at each time step and is therefore computationally more expensive. Consequently, it is applied only to selected cases where higher numerical stability and accuracy are required.
...
\subsection{Measure of $\beta$-Diversity}
    ...
    \begin{align*}\label{eqn:alpha_gamma_dis}
    ...
    \begin{align*}\label{eqn:alpha_gamma_con}
    ...
    We apply these measures to the simulated population distributions to obtain quantitative estimates of $\beta$-diversity. In addition, analytical expressions are derived under the assumption that species distributions follow logistic profiles.